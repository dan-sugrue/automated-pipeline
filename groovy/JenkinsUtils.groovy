...
void buildPackage(versionPropertyFile) {
  ...
  dir("adm/appian-version-client") {
    ...
    bat(script:"version-application.bat -vc_username \"$REPOUSERNAME\" -vc_password \"$REPOPASSWORD\" -package_path ../app-package.zip -local_repo_path ./local-repo -appian_objects_repo_path appian/applications/${APPLICATIONNAME}" )
...
void inspectPackage(customProperties) {
  ...
  if (fileExists("appian/properties/${APPLICATIONNAME}/" + customProperties)) {
	...
	response=bat( script:"curl --location  --request POST \"$inspectionUrl\" --header \"Appian-API-Key: $APIKEY\" --form \"zipFile=@\"./adm/finalPackage.zip\"\" --form \"ICF=@\"./appian/properties/${APPLICATIONNAME}/${customProperties}\"\" --form \"json={\"packageFileName\":\"finalPackage.zip\",\"customizationFileName\":\"$customProperties\"}\"", returnStdout: true).trim()
  } else{
  	response=bat( script:"curl --location  --request POST \"$inspectionUrl\" --header \"Appian-API-Key: $APIKEY\" --form \"zipFile=@\"./adm/finalPackage.zip\"\" --form \"json={\"packageFileName\":\"finalPackage.zip\"}\"", returnStdout: true).trim()
  }
...
void createDeployment(customProperties) {
  ...
  if (fileExists("appian/properties/${APPLICATIONNAME}/" + customProperties)) {
	String response=bat( script:"curl --silent --location  --request POST \"$deploymentUrl\" --header \"Appian-API-Key: $APIKEY\" --form \"zipFile=@\"./adm/finalPackage.zip\"\" --form \"ICF=@\"./appian/properties/${APPLICATIONNAME}/${customProperties}\"\" --form \"json={\"packageFileName\":\"finalPackage.zip\",\"name\":\"$DEPLOYMENTNAME\",\"customizationFileName\":\"$customProperties\"}\"", returnStdout: true).trim()
  }
